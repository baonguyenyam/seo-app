...
export default function Home() {
	...
	const exportWordPressXML = () => {
		...
		let arr = ["___REPLACE___", "___REPLACE_A___", "___REPLACE_B___", "___REPLACE_C___", "___REPLACE_D___", "___REPLACE_E___"];

		for (let i = 0; i < arr.length; i++) {
			let id = Math.floor(Math.random() * 1000000000);

			m = m.replace(new RegExp(arr[i], "g"), nel[i]);
			m = m.replace(/<wp:post_id>(.*?)<\/wp:post_id>/gi, "<wp:post_id>" + id + "</wp:post_id>");
		}
		...
		<>
			...
			<div id="getcnt">
				...
				<section className="flex flex-col gap-4 py-3 md:py-5">
					...
					<div className="flex flex-wrap gap-2">
						{keywords.map((keyword) => (
							<span key={keyword} className="px-3 py-1 bg-gray-200 rounded-full text-sm flex flex-row items-center dark:bg-gray-800">
								{keyword}
								<Popconfirm cancelText="No" okText="Yes" title="Are you sure to delete this keyword?" onConfirm={() => removeKeyword(keyword)}>
									<CloseOutlined className="ml-2" />
								</Popconfirm>
							</span>
						))}
						<button className="px-3 py-1 bg-gray-200 rounded-full text-sm flex flex-row items-center dark:bg-gray-800" onClick={() => openModalKeyword()}>
							<PlusOutlined className="mr-2" />
							Add Keyword
						</button>
					</div>
					<Modal centered keyboard={false} maskClosable={false} open={keywordOpen} title="Add new Keyword" onCancel={handleKeywordCancel} onOk={handleKeywordOk}>
						<input
							ref={inputRef}
				...
				<section className="flex flex-col gap-4 py-3 md:py-5">
					...
					<div className="flex flex-wrap gap-2">
						{subKeywords.map((subKeyword) => (
							<span key={subKeyword} className="px-3 py-1 bg-gray-200 rounded-full text-sm flex flex-row items-center dark:bg-gray-800">
								{subKeyword}
								<Popconfirm cancelText="No" okText="Yes" title="Are you sure to delete this sub keyword?" onConfirm={() => removeSubKeyword(subKeyword)}>
									<CloseOutlined className="ml-2" />
								</Popconfirm>
							</span>
						))}
						<button className="px-3 py-1 bg-gray-200 rounded-full text-sm flex flex-row items-center dark:bg-gray-800" onClick={() => openModalSubKeyword()}>
							<PlusOutlined className="mr-2" />
							Add Keyword
						</button>
					</div>
					<Modal centered keyboard={false} maskClosable={false} open={subKeywordOpen} title="Add new Keyword" onCancel={handleSubKeywordCancel} onOk={handleSubKeywordOk}>
						<input
							ref={inputRef2}
							className="w-full border border-gray-200 rounded p-2 bg-white"
							id="subKeyword"
				...
				<section className="flex flex-col gap-4 py-3 md:py-5">
					...
					<div className="flex flex-wrap gap-2">
						{states.map((state) => (
							<span key={state} className="px-3 py-1 bg-gray-200 rounded-full text-sm flex flex-row items-center dark:bg-gray-800">
								{state}
								<Popconfirm cancelText="No" okText="Yes" title="Are you sure to delete this state?" onConfirm={() => removeState(state)}>
									<CloseOutlined className="ml-2" />
								</Popconfirm>
							</span>
						))}
						<button className="px-3 py-1 bg-gray-200 rounded-full text-sm flex flex-row items-center dark:bg-gray-800" onClick={() => openModalState()}>
							<PlusOutlined className="mr-2" />
							Add Keyword
						</button>
					</div>
					<Modal centered keyboard={false} maskClosable={false} open={stateOpen} title="Add new Keyword" onCancel={handleStateCancel} onOk={handleStateOk}>
						<input
							ref={inputRef3}
							className="w-full border border-gray-200 rounded p-2 bg-white"
							id="state"
				...
				<section className="flex flex-col gap-4 py-3 md:py-5">
					...
					<div className="flex flex-wrap gap-2">
						{cities.map((city) => (
							<span key={city} className="px-3 py-1 bg-gray-200 rounded-full text-sm flex flex-row items-center dark:bg-gray-800">
								{city}
								<Popconfirm cancelText="No" okText="Yes" title="Are you sure to delete this city?" onConfirm={() => removeCity(city)}>
									<CloseOutlined className="ml-2" />
								</Popconfirm>
							</span>
						))}
						<button className="px-3 py-1 bg-gray-200 rounded-full text-sm flex flex-row items-center dark:bg-gray-800" onClick={() => openModalCity()}>
							<PlusOutlined className="mr-2" />
							Add Keyword
						</button>
					</div>
					<Modal centered keyboard={false} maskClosable={false} open={cityOpen} title="Add new Keyword" onCancel={handleCityCancel} onOk={handleCityOk}>
						<input
							ref={inputRef4}
							className="w-full border border-gray-200 rounded p-2 bg-white"
							id="city"
				...
				<section className="flex flex-col gap-4 py-3 md:py-5">
					...
					<div className="flex flex-wrap gap-2">
						{addonOne.map((ad1) => (
							<span key={ad1} className="px-3 py-1 bg-gray-200 rounded-full text-sm flex flex-row items-center dark:bg-gray-800">
								{ad1}
								<Popconfirm cancelText="No" okText="Yes" title="Are you sure to delete this addonOne?" onConfirm={() => removeAddonOne(ad1)}>
									<CloseOutlined className="ml-2" />
								</Popconfirm>
							</span>
						))}
						<button className="px-3 py-1 bg-gray-200 rounded-full text-sm flex flex-row items-center dark:bg-gray-800" onClick={() => openModalAddonOne()}>
							<PlusOutlined className="mr-2" />
							Add Keyword
						</button>
					</div>
					<Modal centered keyboard={false} maskClosable={false} open={addonOneOpen} title="Add new Keyword" onCancel={handleAddonOneCancel} onOk={handleAddonOneOk}>
						<input
							ref={inputRef5}
							className="w-full border border-gray-200 rounded p-2 bg-white"
							id="addonOne"
				...
				<section className="flex flex-col gap-4 py-3 md:py-5">
					...
					<div className="flex flex-wrap gap-2">
						{addonTwo.map((ad2) => (
							<span key={ad2} className="px-3 py-1 bg-gray-200 rounded-full text-sm flex flex-row items-center dark:bg-gray-800">
								{ad2}
								<Popconfirm cancelText="No" okText="Yes" title="Are you sure to delete this addonTwo?" onConfirm={() => removeAddonTwo(ad2)}>
									<CloseOutlined className="ml-2" />
								</Popconfirm>
							</span>
						))}
						<button className="px-3 py-1 bg-gray-200 rounded-full text-sm flex flex-row items-center dark:bg-gray-800" onClick={() => openModalAddonTwo()}>
							<PlusOutlined className="mr-2" />
							Add Keyword
						</button>
					</div>
					<Modal centered keyboard={false} maskClosable={false} open={addonTwoOpen} title="Add new Keyword" onCancel={handleAddonTwoCancel} onOk={handleAddonTwoOk}>
						<input
							ref={inputRef6}
							className="w-full border border-gray-200 rounded p-2 bg-white"
							id="addonTwo"
			...
			<div className="hidden" id="results">
				...
				<section className="flex flex-col gap-4">
					...
					<p className="mt-2 text-sm opacity-60">
						<strong>Code Key:</strong> <code className="a">___REPLACE___</code>, <code className="a">___REPLACE_A___</code>, <code className="a">___REPLACE_B___</code>, <code className="a">___REPLACE_C___</code>, <code className="a">___REPLACE_D___</code>, <code className="a">___REPLACE_E___</code>
					</p>
						...
						<table className="w-full border border-gray-200 dark:border-gray-700">
							<tbody>
								{generatedKeywords.map((keyword) => (
									<tr key={keyword} className="border-b border-gray-200 dark:bg-gray-800 dark:border-gray-700">
